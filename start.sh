...
cd /home/ec2-user/Pre-cadastro-Bradesco-lollapalooza-2025  # Caminho do projeto
...
server {
    listen 80;
    server_name bradesco-pre-cadastro.picbrand.dev.br;
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/bradesco-pre-cadastro.picbrand.dev.br/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/bradesco-pre-cadastro.picbrand.dev.br/privkey.pem;
...
if [ -f "/etc/letsencrypt/live/bradesco-pre-cadastro.picbrand.dev.br/fullchain.pem" ]; then
    echo "✅ Certificado SSL já existe. Pulando a geração."
else
    echo "⚡ Gerando certificado SSL..."
    sudo certbot certonly --nginx -d bradesco-pre-cadastro.picbrand.dev.br --non-interactive --agree-tos -m seuemail@exemplo.com
fi
